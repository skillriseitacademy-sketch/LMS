...
export function ModernistTemplate({ data }: { data: ResumeData }) {
  return (
    <div
      className="bg-white text-gray-800 flex max-w-[800px] w-full min-h-[1056px] shadow-sm"
      style={{ fontFamily: "Inter, sans-serif" }}
    >
      {/* Left Sidebar */}
      <div className="w-1/3 bg-gray-100 p-8 flex flex-col gap-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 leading-tight tracking-tight break-words">
            {data.firstName}
            <br />
            {data.lastName}
          </h1>
          <p className="text-sm font-semibold text-primary uppercase mt-2 tracking-wider">
            {data.title}
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-2 text-sm">
            <span className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[16px] text-gray-400">email</span>{" "}
              {data.email}
            </span>
            <span className="flex items-center gap-2">
