    ...
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur md:px-6">
      ...
      <div className="flex-1">
        {breadcrumb ? (
          ...
          <h1 className="text-display text-base font-semibold">{title}</h1>
        ) : null}
      </div>
      ...
      <Avatar className="h-8 w-8">
        <AvatarFallback className="bg-brand text-brand-foreground text-xs font-semibold">SA</AvatarFallback>
      </Avatar>
    </header>
  );
}
