...
export function RoadmapTree({ initialNode }: RoadmapTreeProps) {
    ...
    try {
      const res = await fetch("/api/roadmap", {
        ...
        headers: {
          ...
          ...(session
            ? { Authorization: `Bearer ${(supabase as any).realtime?.accessToken ?? ""}` }
            : {}),
        },
        body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    node
      .append("circle")
      .attr("fill", (d) =>
        d.data.type === "job" ? "#3b82f6" : d.data.type === "education" ? "#a855f7" : "#10b981",
      )
      .attr("stroke", "#0f172a")
      .attr("stroke-width", 2)
      .attr("r", 5)
    ...
    node
      ...
      .attr("text-anchor", (d) =>
        d.children || d.data._children || d.data.isLoading ? "end" : "start",
      )
      .text((d) => d.data.label)
      .attr("fill", "#cbd5e1")
    ...
    const regeneratableNodes = node.filter(
      (d) => !d.data.isLoading && d.data.type !== "root" && !!(d.data.children || d.data._children),
    );
    regeneratableNodes
      .append("text")
      .attr("dy", "0.31em")
  ...
  return (
    <div
      className="relative w-full h-[650px] bg-[#0f172a] rounded-2xl overflow-hidden border border-slate-800 shadow-xl"
      ref={containerRef}
    >
      <style>{`
      ...
      <div className="absolute top-4 left-4 bg-[#1e293b]/80 backdrop-blur-md px-4 py-2 rounded-lg border border-slate-700 pointer-events-none">
        ...
        <p className="text-xs text-slate-400 mt-1">
          Click nodes to expand/collapse. Zoom and pan to navigate.
        </p>
        <div className="flex items-center gap-3 mt-2 text-[10px] uppercase tracking-wider text-slate-300">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#10b981]"></span> Root
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#a855f7]"></span> Education
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#3b82f6]"></span> Job
          </span>
        </div>
      </div>
      ...
      <div
        className={`absolute bottom-6 left-6 max-w-sm bg-[#1e293b] border border-slate-700 p-4 rounded-xl shadow-2xl transition-opacity duration-200 pointer-events-none ${hoveredNode && hoveredNode.description ? "opacity-100" : "opacity-0"}`}
      >
        {hoveredNode && (
          <>
            <strong className="text-white block mb-1 text-sm">{hoveredNode.label}</strong>
            <span
              className="capitalize text-[10px] font-bold tracking-wider mb-2 block"
              style={{
                color:
            ...
            {hoveredNode.description && (
              <p className="text-xs text-slate-400 leading-relaxed">{hoveredNode.description}</p>
            )}
          </>
        )}
      </div>
