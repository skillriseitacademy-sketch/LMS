...
import { loginSession, type Role } from "@/lib/auth-store";
...
export function AuthShell({ title, sub, cta, alt }: { title: string; sub: string; cta: string; alt: [string, string, string] }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    loginSession({
      ...
      role,
      name: name || (role === "admin" ? "Admin User" : "Sam Adams"),
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 rounded-xl border p-2.5 text-sm font-medium transition-colors ${role === "admin" ? "border-foreground bg-foreground text-background" : "border-border hover:bg-muted"}`}
            >
          ...
          <button type="submit" className="block w-full rounded-xl bg-brand py-2.5 text-center text-sm font-semibold text-brand-foreground hover:opacity-90">{cta}</button>
        </form>
