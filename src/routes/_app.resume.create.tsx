    ...
    <div className="flex-1 flex flex-col h-full transition-all duration-300">
      ...
      <header className="bg-surface border-b border-outline-variant/30 flex justify-between items-center w-full h-16 px-4 md:px-8 sticky top-0 z-40 shrink-0">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center gap-2 bg-surface-container text-on-surface px-4 py-2 rounded-lg font-body-md font-medium hover:bg-surface-container-high transition-all"
          >
            Save to DB
          </button>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-surface-container-low h-[calc(100vh-64px)]">
            ...
            <div className="bg-surface-bright rounded-xl shadow-sm border border-outline-variant/20 overflow-hidden border-l-4 border-l-primary">
              ...
              <div className="p-6 space-y-6">
                {resumeData.experiences.map((exp, idx) => (
                  <div
                    key={exp.id}
                    className="border border-outline-variant/30 rounded-lg p-4 relative group hover:border-primary/50 transition-colors"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="space-y-1">
                        ...
                        <input
                          value={exp.title}
                          onChange={(e) => handleExpUpdate(exp.id, "title", e.target.value)}
                          className="w-full bg-surface-container-lowest border border-outline-variant/50 rounded-lg px-3 py-2 text-on-surface font-body-md focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none"
                          type="text"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <input
                          value={exp.employer}
                          onChange={(e) => handleExpUpdate(exp.id, "employer", e.target.value)}
                          className="w-full bg-surface-container-lowest border border-outline-variant/50 rounded-lg px-3 py-2 text-on-surface font-body-md focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none"
                          type="text"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <input
                          value={exp.startDate}
                          onChange={(e) => handleExpUpdate(exp.id, "startDate", e.target.value)}
                          className="w-full bg-surface-container-lowest border border-outline-variant/50 rounded-lg px-3 py-2 text-on-surface font-body-md focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none"
                          type="month"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <input
                          value={exp.endDate}
                          onChange={(e) => handleExpUpdate(exp.id, "endDate", e.target.value)}
                          className="w-full bg-surface-container-lowest border border-outline-variant/50 rounded-lg px-3 py-2 text-on-surface font-body-md focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none"
                          type="month"
                        />
                      </div>
                    ...
                    <div className="space-y-1 relative">
                      ...
                      <textarea
                        ...
                        onChange={(e) => handleExpUpdate(exp.id, "description", e.target.value)}
        ...
        <section className="hidden lg:flex lg:w-1/2 h-full relative flex-col items-center justify-start p-8 overflow-y-auto bg-surface-variant/30">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-body-sm text-sm text-on-surface">
                  Consider adding metrics to your TechNova experience to increase impact score by
                  ~15%.
                </p>
                <button className="mt-2 text-xs font-label-sm text-primary font-medium hover:underline">
                  Apply suggestion
                </button>
              </div>
            </div>
          ...
          <div
            className="w-[800px] min-h-[1056px] bg-white shadow-xl origin-top scale-[0.6] xl:scale-[0.75] transition-transform flex-shrink-0"
