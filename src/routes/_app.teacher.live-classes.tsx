...
export const Route = createFileRoute("/_app/teacher/live-classes")({
...
function AdminLiveClasses() {
  ...
  const fetchData = async () => {
    ...
    if (teachersRes.error) toast.error("Failed to fetch teachers: " + teachersRes.error.message);
      ...
      if (teachersRes.data && teachersRes.data.length > 0 && !teacherId)
        setTeacherId(session?.id || teachersRes.data[0].id);
    }
    setLoading(false);
  };
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">
              No classes found. Schedule one to get started.
            </p>
          ) : (
            classes.map((c) => (
              <div
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`inline-flex px-2 py-0.5 rounded text-[10px] uppercase font-bold ${c.status === "completed" ? "bg-muted text-muted-foreground" : "bg-brand/10 text-brand"}`}
                  >
                    {c.status}
                  </span>
                </div>
                ...
                <div className="mt-4 space-y-2 flex-1">
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      {new Date(c.start_time || c.scheduled_at).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}{" "}
