      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!currentLevel || !field) {
            return new Response(
              JSON.stringify({ error: "Missing required fields (currentLevel, field)" }),
              { status: 400 },
            );
          }
          ...
          const { object, usage } = await generateObject({
            ...
            schema: z.object({
              nodeId: z
                .string()
                .describe("Echo back the parentNodeId if provided, otherwise 'root'"),
