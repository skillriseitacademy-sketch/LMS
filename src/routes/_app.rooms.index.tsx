...
function RoomsHub() {
        ...
        .then(({ data }) => {
          if (data) {
            const activeRooms = data.filter((room) => {
              const roomAgeHours =
                (new Date().getTime() - new Date(room.created_at).getTime()) / (1000 * 60 * 60);
  ...
  const handleStart = async () => {
    ...
    const roomUrl = `https://meet.jit.si/PlacePro-Room-${newCode}`;
    ...
    if (data) {
      if (isTeacher) {
        // Automatically make it appear in Live Classes
        await supabase.from("live_classes").insert({
          title: `Instant Class by ${session?.name || "Teacher"}`,
          description: "Instant live session started from Rooms",
  ...
  const handleSchedule = async (e: React.FormEvent) => {
    ...
    if (data) {
      if (isTeacher) {
        // Automatically make it appear in Live Classes
        await supabase.from("live_classes").insert({
          title: `Scheduled Class by ${session?.name || "Teacher"}`,
          description: "Scheduled live session started from Rooms",
          teacher_id: session?.id,
          room_url: roomUrl,
          status: "scheduled",
          scheduled_for: scheduledDateIso,
    ...
    <div className="flex flex-col min-h-screen bg-[var(--pp-surface)] pb-24 md:pb-0">
      ...
      <div className="flex-1 w-full max-w-[1280px] mx-auto p-4 md:p-8 flex flex-col gap-6">
        ...
        <div className="text-center mt-4 md:mt-12 mb-8 relative">
          ...
          >
            Start a quick study session or join a peer's room to practice interviews, collaborate on
            code, and study together.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto w-full relative z-10">
          ...
          <div
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor =
                "color-mix(in srgb, var(--pp-primary) 50%, transparent)";
              (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)";
              (e.currentTarget as HTMLElement).style.boxShadow =
                "0 12px 24px -8px color-mix(in srgb, var(--pp-primary) 20%, transparent)";
            }}
            onMouseLeave={(e) => {
          ...
          >
            ...
            <div
              ...
              style={{
                backgroundColor: "color-mix(in srgb, var(--pp-primary) 10%, transparent)",
            ...
            <div className="relative z-10 flex-1 flex flex-col mb-8">
              ...
              <p className="text-sm flex-1" style={{ color: "var(--pp-on-surface-variant)" }}>
                {["teacher", "admin"].includes(session?.role || "")
                  ? "Start a live, automatically recorded class."
                  : "Get a link you can share with peers to collaborate immediately."}
              </p>
            </div>
          ...
          <div
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor =
                "color-mix(in srgb, var(--pp-secondary) 50%, transparent)";
              (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)";
              (e.currentTarget as HTMLElement).style.boxShadow =
                "0 12px 24px -8px color-mix(in srgb, var(--pp-secondary) 20%, transparent)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor = "var(--pp-outline-variant)";
              (e.currentTarget as HTMLElement).style.transform = "none";
          ...
          >
            ...
            <div
              className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 relative z-10 transition-transform group-hover:scale-110"
              style={{
                backgroundColor: "color-mix(in srgb, var(--pp-secondary) 10%, transparent)",
              ...
              <input
                ...
                onFocus={(e) => {
                  ...
                  (e.currentTarget as HTMLInputElement).style.boxShadow =
                    "0 0 0 3px color-mix(in srgb, var(--pp-secondary) 15%, transparent)";
                }}
                onBlur={(e) => {
                  (e.currentTarget as HTMLInputElement).style.borderColor =
                    "var(--pp-outline-variant)";
          ...
          <div
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor =
                "color-mix(in srgb, var(--pp-tertiary) 50%, transparent)";
              (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)";
              (e.currentTarget as HTMLElement).style.boxShadow =
                "0 12px 24px -8px color-mix(in srgb, var(--pp-tertiary) 20%, transparent)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor = "var(--pp-outline-variant)";
              (e.currentTarget as HTMLElement).style.transform = "none";
          ...
          >
            ...
            <div
              className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 relative z-10 transition-transform group-hover:scale-110"
              style={{
                backgroundColor: "color-mix(in srgb, var(--pp-tertiary) 10%, transparent)",
            ...
            <div className="relative z-10 flex-1 flex flex-col mb-8">
              ...
              <p className="text-sm flex-1" style={{ color: "var(--pp-on-surface-variant)" }}>
                {["teacher", "admin"].includes(session?.role || "")
                  ? "Plan an automatically recorded class for later."
                  : "Plan a peer meeting for later and invite others."}
              </p>
            </div>
              ...
              {upcomingRooms.map((room) => (
                ...
                >
                  <div>
                    ...
                    <p className="font-mono text-sm text-[var(--pp-on-surface-variant)]">
                      Code: {room.room_code}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() =>
                        navigate({ to: "/rooms/$roomCode", params: { roomCode: room.room_code } })
                      }
