...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const BUCKET_MAP: Record<string, { bucket: string; publicUrl: string }> = {
      post: {
        bucket: process.env.R2_BUCKET_MEDIA || "placepro-media",
      ...
      interview: {
        bucket: process.env.R2_BUCKET_RECORDINGS || "placepro-recordings",
        publicUrl: process.env.R2_PUBLIC_URL_RECORDINGS || "",
      },
      course: {
        bucket: process.env.R2_BUCKET_CONTENT || "placepro-content",
        publicUrl: process.env.R2_PUBLIC_URL_CONTENT || "",
      },
    };
